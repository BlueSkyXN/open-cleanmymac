...
- `clean`/`purge`/`analyze` 新增 `--interactive` 显式进入全屏 TUI：需要 stdin/stdout
  ...
  `openclean.display_mode`，契约矩阵见新增 [docs/MODES.md](docs/MODES.md)。
- clean/purge/analyze 共用统一扫描页：`Space` 请求暂停/继续（“已暂停”仅在实际任务
  到达暂停点后显示），`Q` 取消审阅（exit 0）、`Ctrl-C` 中断（exit 130）；取消后等待
  扫描线程收尾。clean/purge 显示任务清单与任务进度，Analyze 显示已检查一级项数与
  当前处理对象；扫描转审阅边界清理过期按键，Q 优先、Space/Enter 不泄漏进审阅页。
- Analyze 浏览新增三态选择标记（`[ ]` 未选、`[x]` 已直接选择、`[-]` 目录内有独立选择、
  `[x] 随上级` 被祖先选择覆盖）与全局/当前目录两层选择汇总；目录整体或被上级选择时
  如实标注，不拆算父项计量。精确选择集合与执行语义不变。
- Clean/Purge 逐项列表显示本分类已选汇总；主菜单 More 新增“命令速查”（curses 与
  行式菜单均可用）。

### Changed

- 无 `--yes` 的汇总页标题改为“选择预览”；Analyze 执行确认明确写“移动到同卷废纸篓”。
- 面向用户的列表把 `safe/confirm/critical` 显示为“可选择/需确认/需重点确认”；内部
  枚举与 JSON 字段不变，详情页保留原始枚举。
- 文本空结果区分“扫描完成，没有发现候选”“扫描不完整，部分位置无法读取”“用户取消”；
  仅对完整扫描使用“未发现/没有发现”结论。Analyze 的 E 问题列表区分“问题”与“提示”。
- 主菜单 Analyze 先选择分析范围；显式 `analyze [path]` 的参数及默认 `/` 保持。
- Analyze 扫描在后台运行，先显示加载状态并分批反馈；Q/Ctrl-C 在等待线程收尾前传递取消，
  保留已完成候选，取消不显示为扫描成功。SIGINT 返回 130，curses 退出恢复终端。
- 返回目录复用有界会话缓存与光标，R 刷新；提交选择前重新分析来源，变化或不可执行项撤销，
  执行仍需既有授权、critical 二次确认和 live 复核。缓存不持久化。
- Analyze 新增 I 详情、E 全部问题及明确只读原因；补充仅浏览的零占用行，未知容量不伪装为零。
  正常提交承接当前及所选来源的完整性，部分结果返回 1；主动取消审阅保持既有语义。
- 同次保护上下文复用静态路径表；Analyze 采用有界批次调度，保留全局硬链接归属、云与文件系统
  边界。默认单 worker；内部基准支持并发对照，不新增 CLI 参数、依赖或清理范围。
- 新增临时 HOME 性能基准及异步/PTY 回归验证。JSON schema v2 与 top 的显示截断语义保持。
- 兼容期默认路由保持不变：TTY 下自动进入 TUI 的行为未迁移；相关迁移将单独决策。
...
- Analyze 当前处理路径来自实际候选/目录访问，不把排队的批次标成并行执行；
  已启动扫描后的界面失败明确返回错误，不再回退并重扫。
- 任务图在中断时先通知扫描取消，再等待线程，防止程序化 CLI 的 SIGINT 卡在收尾；
  任务提交阶段也覆盖取消，有限周期等待确保送达工作线程的 SIGINT 能由主线程及时处理。
  非交互扫描的信号处理器只记录请求，扫描检查点协作退出，收尾后再返回 130，避免异步
  KeyboardInterrupt 打断线程锁操作；扫描结束恢复原信号处理器。
  Analyze 本层批量选择一次性合并，避免逐候选重复遍历全部已有选择。
- Analyze 取消时，即使停止画面绘制失败或线程收尾期间再次收到 Ctrl-C，也等待工作线程的
  完成信号后退出；终端错误不掩盖原有中断，输入模式始终恢复。
- 子目录分析失败后，无论父目录能否命中会话缓存，都保留并显示失败原因。
- 大量问题记录不再导致每次按键重复逐行比对全部路径；问题详情按窗口需要增量排版，
  滚动复用已排版文本，窗口宽度变化时重新计算，保留完整问题内容。
- 终端缺少 TERM 环境变量时，扫描界面在初始化 curses 前即拒绝并回退文本流程；
  避免同进程内反复失败的 initscr 在 macOS 上直接中止进程。

## [0.24.0a3] - 2026-09-20

Alpha 功能版；新增只读大文件扫描与 Codex 浏览器缓存识别，打磨 CLI/TUI 终端呈现并适配
浅色与深色终端。生产清理策略、选择语义和 JSON schema v2 不变；新增扫描项默认不预选，
清理动作仍需 `--yes` 与既有保护条件。
...
- 清理结果不再把永久删除计量称为“永久释放”；文本回执说明操作计量不等于磁盘可用空间
  净增量，TUI 确认页同步准确描述不可恢复的删除操作，JSON 字段和执行权限不变。
- Purge 不再将整个 `.vitepress` 当作可重建产物，只识别其一级 `cache`、`dist`，
  保留配置、主题源码和相似名称目录；年龄预选与精确选择规则不变。
- Clean、Purge、Analyze 与执行器共用应用/updater 范围保护；静态归属、已注册扫描点和
  有界 bundle ID 查询覆盖精确根、后代及包含实际已知子项的父容器，不匹配相似名称。
  同路径分类合并保留运行保护和不可执行原因，不因环境变量或 Analyze 入口绕过保护。
- 含 updater 暂存区的父目录/子目录要求重新扫描并精确审阅受支持的 updater 根；
  同路径合并保留完整版本证据、`critical` 与精确选择要求。批量预检及逐项移动前重新判定，
  即使旧扫描没有进程标记或暂存状态，也能阻断新出现的运行对象、暂存包和版本变化。
  扩展范围探测不穿过 symlink、云占位或忽略/保护目录，普通缓存和已确认残留的动作保留。
- `ignore list/add/remove` 遇到非 UTF-8 规则文件时返回 `rules_error`/exit 2，
  JSON 与脱敏错误输出保持可解析，原文件不被覆盖。
- `scan`、`ignore` 和 `config` 读取嵌套过深的 JSON 时返回现有 `rules_error`/
  `config_error` 与 exit 2，不再泄漏 `RecursionError` 堆栈或覆盖原文件；脱敏输出同样可解析。
- 修复跨扫描点、顶层文件及普通域与项目域合并时的硬链接重复计量。
  共享容量只归入一个候选及其待扣除的父项，其它路径仍可独立审阅和选择；
  保留父子重叠计量与 JSON schema v2，不把 inode 计量当作实际释放空间承诺。

### Changed

- `large` 拒绝含符号链接的扫描路径时，错误信息附上解析后的真实路径提示
  （如 `/tmp` 可改用 `/private/...` 重试）；仍逐级拒绝，不自动跟随链接或放开保护。
- `large` 文本报告在阈值处同时显示精确字节数，区分十进制输入（KB/MB）与
  二进制缩写显示，避免两者基数混淆。
- 改善 CLI/TUI 视觉：文本候选与完整路径分行，摘要区分发现、可执行、选择和阻断量；
  终端界面使用整行焦点、文字状态、中文列宽对齐、长文本省略和快捷键换行。
  Analyze 显示文件名、占比条及首个阻断问题；主菜单与确认页统一层级。
  `OPENCLEAN_THEME=auto` 默认沿用终端前景/背景及反白，适配白底和深色 iTerm2/Terminal，
  避免粗体颜色与淡化提示影响可读性；light/dark 可显式启用 256 色状态配色，NO_COLOR 优先。
  补充浅色/深色合成预览与参考背景对比度检查；小于 48×14 时保留选择并暂停隐藏操作。
  JSON、执行保护和确认步骤保持兼容。
...
目录依据与版本边界见能力地图；本节变化已随 0.24.0a3 发布，不代表所有安装版本的实机验收。
