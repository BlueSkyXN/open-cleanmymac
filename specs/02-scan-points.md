...
Codex:     .codex/cache/codex_apps_server_info  .codex/cache/codex_apps_tools  .codex/tmp
...
chrome-devtools-mcp: .cache/chrome-devtools-mcp/chrome-profile/**(同上浏览器缓存子项)
杂项:      *.log
```
...
```
行为契约：递归统计目录大小 → 大文件/大目录排序 → 区分"真实占用"与"可清除/可 purge 空间"。
